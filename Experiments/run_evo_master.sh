...
#DATASETS=(mh01)
#GT_PATHS=(~/dataset/euroc/MH_01/state_groundtruth_estimate0/data.csv)
# --------------- helpers ---------------
...
usage() {
  cat <<EOF
Usage: $(basename "$0") [--ape] [--only NAME|glob] [--dry] [--rpe-align none|se3|sim3]
...
main() {
  ...
  for d in "${dirs[@]}"; do
    ...
    # --- RPE (rotation) ---
    echo "[RPE-rot] evo_rpe <ref> $gt <est> ${RPE_ALIGN_FLAGS} -a -s -r rot_part -d 1 -u f"
    if [[ $DO_DRY -eq 0 ]]; then
      bash ./run_evo.sh rpe "$d" "$gt" "${name}__ROT" ${RPE_ALIGN_FLAGS} -r rot_part -d 1 -u f || true
  ...
  echo ""; echo "All done."
  echo "Translation RPE -> ./PoseErrors/RPE/<name>.txt"
  echo "Rotation    RPE -> ./PoseErrors/RPEr/<name>.txt"
  if [[ $DO_APE -eq 1 ]]; then echo "APE            -> ./PoseErrors/APE/<name>.txt"; fi
}
